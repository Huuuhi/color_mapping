...
namespace Simplify
{
	...
	// lhy
	void load_triangle(std::vector<glm::vec3>& in_vertices, std::vector<unsigned int>& in_indices, std::vector<float>& in_scalars)
	{
		vertices.clear();
		triangles.clear();
		for(auto ver : in_vertices)
		{
			Vertex v;
			v.p.x = ver.x;
			v.p.y = ver.y;
		...
		for(int i = 0; i < in_indices.size(); i+=3)
		{
			...
			for (int j = 0; j < 3; j++)
			{
				...
				t.uvs[j].x = in_scalars[in_indices[i + j]];
				t.uvs[j].y = 0;
				t.uvs[j].z = 0;
			}
			t.attr = TEXCOORD;
	...
	void write_triangle(std::vector<glm::vec3>& out_vertices, std::vector<unsigned int>& out_indices, std::vector<float>& out_scalars)
	{
		out_vertices.clear();
		out_indices.clear();
		out_scalars.clear();
		for(int i = 0; i < vertices.size(); i++)
		{
			...
			glm::vec3 ver;
			ver.x = vertices[i].p.x;
			ver.y = vertices[i].p.y;
			ver.z = vertices[i].p.z;
			out_vertices.emplace_back(ver);
			// 将当前顶点的标量值（uvs[j].x）存储到 out_scalars 中，注意与顶点数组的索引相对应
			float scalarValue = 0.0f;  // 默认值，如果找不到uvs[j].x
			for (auto t : triangles)
			{
				
				if (t.v[0] == i)
				{
					scalarValue = t.uvs[0].x;  
					break;
				}
				else if (t.v[1] == i)
				{
					scalarValue = t.uvs[1].x; 
					break;
				}
				else if (t.v[2] == i)
				{
					scalarValue = t.uvs[2].x; 
					break;
				}
			}
			out_scalars.emplace_back(scalarValue);
	...
	void start(std::vector<glm::vec3>& in_vertices, std::vector<unsigned int>& in_indices, std::vector<float>& in_scalars,
		std::vector<glm::vec3>& out_vertices, std::vector<unsigned int>& out_indices, std::vector<float>& out_scalars,
		double reduceFraction = 0.5, double agressiveness = 7.0)
	{
		load_triangle(in_vertices, in_indices, in_scalars);
		...
		//simplify_mesh_lossless(true);
		if (triangles.size() >= startSize) {
			throw std::runtime_error("Unable to reduce mesh.\n");
		}
		write_triangle(out_vertices, out_indices, out_scalars);
